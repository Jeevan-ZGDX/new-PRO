      ...
      {data.map((dept) => (
        <div key={dept.name} className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-700 dark:text-zinc-300">{dept.name}</span>
            <span className="text-sm text-gray-500 dark:text-zinc-400">{dept.count}</span>
          </div>
