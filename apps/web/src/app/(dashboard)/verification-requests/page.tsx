      ...
      <Card>
        <div className="p-4">
          ...
          <div className="flex items-center gap-2 text-xs text-gray-500">
            ...
            {sections.map(s => (
              <span key={s} className="px-2 py-0.5 bg-gray-100 border text-gray-700 rounded-md font-medium">{year.charAt(0)}{s}</span>
            ))}
          </div>
        </div>
      </Card>
          ...
          <div key={vr.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            ...
            >
              ...
              <p className="text-xs text-gray-500 mt-2 ml-[52px]">
                <span className="font-medium text-gray-700">Competition:</span> {vr.competitionTitle}
              </p>
            </div>
              ...
              <div className="border-t border-gray-100 bg-gray-50/50">
                <div className="p-4 space-y-4">
                  {vr.emailProof ? (
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          ...
                          <div className="p-3 bg-white border border-gray-200 rounded-xl">
                            ...
                            <p className="text-sm text-gray-900 font-mono text-xs break-all">{vr.emailProof.from}</p>
                          </div>
                          <div className="p-3 bg-white border border-gray-200 rounded-xl">
                            <p className="text-xs text-gray-400 uppercase tracking-wider mb-1">To</p>
                            <p className="text-sm text-gray-900 font-mono text-xs break-all">{vr.emailProof.to}</p>
                          </div>
                          <div className="p-3 bg-white border border-gray-200 rounded-xl md:col-span-2">
                    ...
                    <div className="text-center py-4 text-xs text-gray-400">No email proof details attached</div>
                  )}
          ...
          {(['all', 'pending', 'verified'] as const).map(f => (
            <button key={f} onClick={() => setStatusFilter(f)}
              className={`px-4 py-2 text-sm font-medium rounded-xl transition-colors capitalize ${
                statusFilter === f ? 'bg-[#38BDF8] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'
              }`}
            >
