...
import { Trophy, Star, Search, Award, Users } from 'lucide-react'
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium transition-all duration-200 hover:scale-[1.02] ${
                    activeTab === tab.key
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-zinc-800 text-gray-500 dark:text-zinc-400">
                    ...
                    <th className="text-left text-xs font-medium uppercase tracking-wider px-4 py-3">Competitions</th>
                  ...
                  {overallLoading ? (
                    ...
                    overallData.map((entry) => (
                      <tr key={entry.rank} className="border-b border-gray-100 dark:border-zinc-800/60">
                        ...
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-3 transition-transform duration-200 hover:scale-[1.02] origin-left cursor-pointer">
                            <div className="w-8 h-8 rounded-full bg-accent/10 flex items-center justify-center shrink-0">
                              <Star className="w-4 h-4 text-accent" />
                            </div>
                            <div>
                              <p className="text-sm font-medium text-gray-900 dark:text-white transition-all duration-200">{entry.studentName}</p>
                        ...
                        <td className="px-4 py-4 text-right">
                          <span className="text-sm font-semibold text-accent inline-block transition-transform duration-200 hover:scale-105">{entry.points}</span>
                        </td>
            ...
            <div className="space-y-6">
              {selectedDept ? (
                <div>
                  ...
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Section {selectedDept} Rankings</h3>
                  <SectionDetailTable sectionId={selectedDept} />
                </div>
              ) : (
                <>
                  <p className="text-sm text-gray-500 dark:text-zinc-400">Click a section to view student rankings</p>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {overallData && overallData.length > 0 ? (
                      (() => {
                        ...
                        return sections.map(sec => {
                          ...
                          return (
                            <div
                              key={sec}
                              className="p-4 bg-gray-50 dark:bg-zinc-800/40 border border-gray-200 dark:border-zinc-700/50 rounded-xl transition-transform duration-200 hover:scale-[1.03] cursor-pointer"
                              onClick={() => setSelectedDept(sec)}
                            >
                              <p className="text-base font-bold text-gray-900 dark:text-white">{sec}</p>
                              <p className="text-xs text-gray-500 dark:text-zinc-400 mt-1">{students.length} students</p>
                      ...
                      <div className="col-span-full text-center py-12 text-gray-500 dark:text-zinc-500">No section data available</div>
                    )}
                  </div>
                </>
              )}
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-3">
                <select
                  ...
                  className="flex-1 px-4 py-2 bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl text-sm text-gray-800 dark:text-zinc-200 focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent transition-transform duration-200 hover:scale-[1.01]"
                >
...
function SectionDetailTable({ sectionId }: { sectionId: string }) {
  ...
  if (isLoading) {
    return (
      <div className="space-y-3">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="h-12 bg-gray-100 dark:bg-zinc-800/50 rounded-xl animate-pulse" />
        ))}
      </div>
    )
  }
  ...
  if (students.length === 0) {
    return <div className="text-center py-12 text-gray-500 dark:text-zinc-500">No students in this section</div>
  }
      ...
      <table className="w-full">
        <thead>
          <tr className="border-b border-gray-200 dark:border-zinc-800 text-gray-500 dark:text-zinc-400">
            ...
            <th className="text-right text-xs font-medium uppercase tracking-wider px-4 py-3">Registered</th>
          </tr>
        </thead>
        <tbody>
          {sorted.length > 0 ? (
            sorted.map((reg, idx) => (
              <tr key={reg.id} className="border-b border-gray-100 dark:border-zinc-800/60">
                ...
                <td className="px-4 py-4">
                  <p className="text-sm font-medium text-gray-900 dark:text-white transition-transform duration-200 hover:scale-[1.02] origin-left cursor-pointer">{reg.userName}</p>
                </td>
                <td className="px-4 py-4">
                ...
                <td className="px-4 py-4 text-right text-sm text-gray-600 dark:text-zinc-400">
            ...
            <tr>
              <td colSpan={5} className="text-center py-12 text-gray-500 dark:text-zinc-500">No registrations yet</td>
            </tr>
          )}
        </tbody>
      </table>
