      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {competitions.map((comp: any) => {
            ...
            return (
              <div key={comp.id}
                className="bg-white border border-gray-200 rounded-2xl overflow-hidden hover:shadow-lg hover:border-accent/30 transition-all cursor-pointer group"
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {user?.role === 'super_admin' && (
                        <button onClick={(e) => { e.stopPropagation(); router.push(`/create-competition?edit=${comp.id}`) }}
                          className="flex items-center gap-1 px-2.5 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-xs font-medium text-gray-500 hover:text-accent transition-colors"
                        >
